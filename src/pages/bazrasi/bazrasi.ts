...
export class BazrasiPage {
    ...
    createForm() {
        this.baseForm = this.fb.group({
            _id: [''],
            customer_code: ['', [CustomValidator.isCodeMeli, Validators.required, Validators.minLength(10), Validators.maxLength(10)]],
    ...
    uploadFile() {
        ...
        let options: FileUploadOptions = {
            fileKey: 'file',
            fileName: this.baseForm.get('customer_code').value + '.jpg',
            chunkedMode: false,
        }
        ...
        this.mediaCapture.captureAudio(options)
            .then(
                (data1: MediaFile[]) => {
                    ...
                    let options: FileUploadOptions = {
                        fileKey: 'file',
                        fileName: this.baseForm.get('customer_code').value + '.' + data1[0].name.split('.')[1],
                        chunkedMode: false,
                    }
                    fileTransfer.onProgress((progressEvent: ProgressEvent) => {
                        this.ng_zone.run(() => {
